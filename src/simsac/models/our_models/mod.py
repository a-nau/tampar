...
def deconvPAC(
    in_planes, out_planes, kernel_size=5, stride=2, padding=2, output_padding=1
):
    from utils.pacnet import PacConvTranspose2d

    return PacConvTranspose2d(
        in_planes, out_planes, kernel_size, stride, padding, output_padding, bias=True
    )
...
class OpticalFlowEstimator(nn.Module):
    def __init__(self, in_channels, batch_norm):
        ...
        self.conv_0 = conv(
            in_channels, 128, kernel_size=3, stride=1, batch_norm=batch_norm
        )
        self.conv_1 = conv(
            in_channels + dd[0], 128, kernel_size=3, stride=1, batch_norm=batch_norm
        )
        self.conv_2 = conv(
            in_channels + dd[1], 96, kernel_size=3, stride=1, batch_norm=batch_norm
        )
        self.conv_3 = conv(
            in_channels + dd[2], 64, kernel_size=3, stride=1, batch_norm=batch_norm
        )
        self.conv_4 = conv(
            in_channels + dd[3], 32, kernel_size=3, stride=1, batch_norm=batch_norm
        )
...
def conv_blck(
    in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, bn=False
):
    if bn:
        return nn.Sequential(
            nn.Conv2d(
...
class ConvDecoder(CorrespondenceMapBase):
    def __init__(
        ...
        bn=False,
        out_channels=32,
        inter_channels=[128, 128, 96, 64, 32],
    ):
        super().__init__(in_channels, bn)
        chan = inter_channels
        self.conv0 = conv_blck(in_channels, chan[0], bn=bn)
