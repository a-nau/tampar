...
from src.simsac.models.feature_backbones.ResNet_features import ResNetPyramid
from src.simsac.models.feature_backbones.VGG_features import VGGPyramid
from src.simsac.models.our_models.consensus_network_modules import (
    FeatureCorrelation,
    MutualMatching,
    NeighConsensus,
)
from src.simsac.models.our_models.mod import (
    CMDTop,
    ConvDecoder,
    CorrelationVolume,
    FeatureL2Norm,
...
from src.simsac.models.correlation import correlation  # the custom cost volume layer
...
class SimSaC_Model(nn.Module):
    def __init__(
        ...
        if self.cyclic_consistency:
            ...
            self.NeighConsensus = NeighConsensus(
                use_cuda=True, kernel_sizes=ncons_kernel_sizes, channels=ncons_channels
            )
        else:
        ...
        if dense_connection:
            ...
            self.decoder3 = OpticalFlowEstimatorNoDenseConnection(
                in_channels=od, batch_norm=batch_norm
            )
        ...
        if dense_connection:
            ...
            self.decoder2 = OpticalFlowEstimatorNoDenseConnection(
                in_channels=od, batch_norm=batch_norm
            )
            input_to_refinement = 32
        if self.decoder_inputs == "corr_flow_feat":
        ...
        if dense_connection:
            ...
            self.decoder1 = OpticalFlowEstimatorNoDenseConnection(
                in_channels=od, batch_norm=batch_norm
            )
            input_to_refinement = 32

        self.l_dc_conv1 = conv(
            input_to_refinement,
            128,
            kernel_size=3,
            stride=1,
        ...
        self.change_dec4 = ConvDecoder(
            in_channels=512 + 512 + 256, bn=batch_norm, out_channels=self.num_class
        )
        if self.use_pac:
            self.change_deconv4 = deconvPAC(
                self.num_class,
                self.num_class,
                kernel_size=5,
                stride=2,
            ...
            self.change_deconv4 = deconv(
                self.num_class, self.num_class, kernel_size=4, stride=2, padding=1
            )

        self.change_dec3 = ConvDecoder(
            in_channels=256 + 256 + 81, bn=batch_norm, out_channels=self.num_class
        )
        if self.use_pac:
            self.change_deconv3 = deconvPAC(
                self.num_class,
                self.num_class,
                kernel_size=5,
                stride=2,
        ...
        if self.use_pac:
            ...
            self.change_deconv2 = deconv(
                self.num_class, self.num_class, kernel_size=4, stride=2, padding=1
            )
        # self.change_deconv2 = deconv(self.num_class, self.num_class, kernel_size=4, stride=2, padding=1)

        self.change_dec1 = ConvDecoder(
            in_channels=128 + 128 + 81, bn=batch_norm, out_channels=self.num_class
        )
        if self.use_pac:
            self.change_deconv1 = deconvPAC(
                self.num_class,
                self.num_class,
                kernel_size=5,
                stride=2,
    ...
    def coarsest_resolution_flow(self, c14, c24, h_256, w_256, return_corr=False):
        ...
        if self.cyclic_consistency:
            corr4d = self.corr(
                self.l2norm(c24), self.l2norm(c14)
            )  # first source, then target
            # run match processing model
            corr4d = MutualMatching(corr4d)
            corr4 = corr4d.squeeze(1).view(
                b, c24.shape[2] * c24.shape[3], c14.shape[2], c14.shape[3]
            )
            ...
            corr4d = MutualMatching(
                corr4d
            )  # size is [b, 1, hsource, wsource, htarget, wtarget]
            corr4 = corr4d.squeeze(1).view(
                c24.shape[0], c24.shape[2] * c24.shape[3], c14.shape[2], c14.shape[3]
            )
        else:
    ...
    def multiclass2binary_softmax(self, multiclass_changemap):
        binarymap = torch.stack(
            [multiclass_changemap[:, 0], torch.sum(multiclass_changemap[:, 1:], dim=1)],
            dim=1,
        )
        binarymap = F.softmax(binarymap, dim=1)[:, 1]
        return binarymap[:, None, ...]

    def forward_sigle_ref(
        self, im_target, im_source, im_target_256, im_source_256, disable_flow=None
    ):
        ...
        # RESOLUTION 256x256
        # level 16x16
        flow4, corr4_changehead = self.coarsest_resolution_flow(
            c14, c24, h_256, w_256, return_corr=True
        )  # (8,2,16,16)
        ...
        # ORIGINAL RESOLUTION
        up_change3 = F.interpolate(
            input=change3,
            size=(int(h_full / 8.0), int(w_full / 8.0)),
            mode="bilinear",
            align_corners=False,
        )
        ...
        if self.iterative_refinement and self.evaluation:
            ...
            if nbr_extra_layers == 0:
                ...
                for n in range(nbr_extra_layers):
                    ...
                    up_flow3 = F.interpolate(
                        input=flow3,
                        size=(int(h_full * ratio), int(w_full * ratio)),
                        mode="bilinear",
                        align_corners=False,
                    )
                    c23_bis = torch.nn.functional.interpolate(
                        c22,
                        size=(int(h_full * ratio), int(w_full * ratio)),
                        mode="area",
                    )
                    c13_bis = torch.nn.functional.interpolate(
                        c12,
                        size=(int(h_full * ratio), int(w_full * ratio)),
                        mode="area",
                    )
                    warp3 = warp(c23_bis, up_flow3 * div * ratio, disable_flow)
                    corr3 = correlation.FunctionCorrelation(
                        tensorFirst=c13_bis, tensorSecond=warp3
                    )
                    corr3_changehead = self.l2norm(F.relu(corr3))
                    corr3 = self.leakyRELU(corr3)
                    if self.decoder_inputs == "corr_flow_feat":
        ...
        change2 = self.change_dec2(
            x1=c12, x2=warp2, x3=corr2_changehead, mask=up_change3_binary
        )
        ...
        if self.refinement_at_all_levels:
            x = self.dc_conv4_level2(
                self.dc_conv3_level2(self.dc_conv2_level2(self.dc_conv1_level2(x2)))
            )
            flow2 = flow2 + self.dc_conv7_level2(
                self.dc_conv6_level2(self.dc_conv5_level2(x))
            )
        ...
        change1 = self.change_dec1(
            x1=c11, x2=warp1, x3=corr1_changehead, mask=up_change2_binary
        )
        ...
        if self.evaluation:
            return flow1, change1
            # return flow1, up_change1
        else:
            return {
    ...
    def forward_multiple_ref(
        ...
        # level 16x16
        flow4, corr4_changehead = self.coarsest_resolution_flow(
            c14.repeat(20, 1, 1, 1), c24, h_256, w_256, return_corr=True
        )  # (8,2,16,16)
        up_flow4 = self.deconv4(flow4)  # (8,2,32,32)
        change4 = self.change_dec4(
            c14.repeat(20, 1, 1, 1), c24, corr4_changehead
        )  # (b,5,16,16)
        del c14
        if self.use_pac:
            aligned_imgs_4 = self.resize_align_images(
                im_source, im_target, size=(32, 32), flow=None
            )
        ...
        # constrained correlation now
        corr3 = correlation.FunctionCorrelation(
            tensorFirst=c13.repeat(20, 1, 1, 1), tensorSecond=warp3
        )
        corr3_changehead = self.l2norm(F.relu(corr3))
        change3 = self.change_dec3(
            c13.repeat(20, 1, 1, 1), warp3, corr3_changehead, mask=up_change4_binary
        )
        del c13
        # ORIGINAL RESOLUTION
        up_change3 = F.interpolate(
        ...
        ratio = 1.0 / 8.0
        warp2 = warp(c22, up_flow3 * div * ratio, disable_flow)
        corr2 = correlation.FunctionCorrelation(
            tensorFirst=c12.repeat(20, 1, 1, 1), tensorSecond=warp2
        )
        corr2_changehead = self.l2norm(F.relu(corr2))
        change2 = self.change_dec2(
            x1=c12.repeat(20, 1, 1, 1),
            x2=warp2,
            x3=corr2_changehead,
            mask=up_change3_binary,
        )
        del c12
        if self.use_pac:
            aligned_imgs_2 = self.resize_align_images(
                im_source,
                im_target,
                size=(int(h_full / 4.0), int(w_full / 4.0)),
        ...
        ratio = 1.0 / 4.0
        warp1 = warp(c21, up_flow2 * div * ratio, disable_flow)
        corr1 = correlation.FunctionCorrelation(
            tensorFirst=c11.repeat(20, 1, 1, 1), tensorSecond=warp1
        )
        corr1_changehead = self.l2norm(F.relu(corr1))
        change1 = self.change_dec1(
            x1=c11.repeat(20, 1, 1, 1),
            x2=warp1,
            x3=corr1_changehead,
            mask=up_change2_binary,
        )
        del c11
        # print(corr1_changehead.shape,c11.shape,warp1.shape,change1.shape)#,change1)
        if self.use_pac:
            aligned_imgs_1 = self.resize_align_images(
                im_source,
                im_target,
                size=(int(h_full / 2.0), int(w_full / 2.0)),
        ...
        if self.evaluation:
            ...
            return {
                "flow": (
                    [flow4[idx][None, ...], flow3[idx][None, ...]],
                    [flow2[idx][None, ...], flow1[idx][None, ...]],
                ),
                "change": (
                    [change4[idx][None, ...], change3[idx][None, ...]],
                    [change2[idx][None, ...], change1[idx][None, ...]],
                ),
                # 'change':([up_change4,up_change3],[up_change2,up_change1])
            }
    ...
    def forward(
        ...
        if self.vpr_candidates:
            return self.forward_multiple_ref(
                im_target, im_source, im_target_256, im_source_256, disable_flow=None
            )
        else:
            return self.forward_sigle_ref(
                im_target, im_source, im_target_256, im_source_256, disable_flow=None
            )

    def calc_flow_std(self, flow, patch_size=16, patch_stride=16):
        # flow: B 2 H W
        flow_patches = flow.unfold(2, patch_size, patch_stride).unfold(
